...
const WorksPage = () => {
    ...
    const [isMobileServicesOpen, setIsMobileServicesOpen] = useState(false); // Added for mobile accordion

    // --- REFS ---
    // REWORKED: Ref to manage the timer for the hover-delay on the dropdown
    ...
    const toggleMobileMenu = () => {
        ...
        // Reset mobile services dropdown when closing the main menu
        if (!nextState) {
            setIsMobileServicesOpen(false);
        }
    };

    // REWORKED: Handlers for a professional hover menu with a close delay
    const handleMouseEnter = () => {
        clearTimeout(dropdownTimerRef.current); // Cancel any pending close timer
        setIsServicesDropdownOpen(true);
    };

    const handleMouseLeave = () => {
        // Set a timer to close the dropdown, giving the user time to move their mouse to it
    ...
    // --- DATA ---
    // Consistent data for navigation and footer components
    const servicesLinks = [
        {
        ...
        },
        {
            ...
            // REWORKED: Using consistent icon from reference
            icon: <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
    ...
    useEffect(() => {
        ...
        const elementsToAnimate = document.querySelectorAll('.animate-entry, .animate-word');
        elementsToAnimate.forEach((el) => {
            observer.observe(el);
        });

        return () => elementsToAnimate.forEach(el => el && observer.unobserve(el));
    }, []);
    ...
    transition: opacity 0.3s ease; 
                    ...
                    /* --- WORKS PAGE STYLES --- */
                    .works-section { padding: 4rem 1.5rem; max-width: 900px; margin: 0 auto; text-align: center; }
                    .works-section h1 { font-size: clamp(2.5rem, 6vw, 4rem); font-weight: 700; color: var(--brand-dark); margin: 0 0 1.5rem; line-height: 1.2; }
                    .works-section h1 .highlight { color: var(--brand-primary); }
                    .works-section p { font-size: clamp(1rem, 2.5vw, 1.25rem); font-weight: 400; color: #555; line-height: 1.7; margin: 0 auto 2rem; max-width: 750px; }
                    .works-section p strong { font-weight: 600; color: var(--brand-primary); }
                    .works-section .subtitle { font-size: clamp(1.1rem, 3vw, 1.375rem); font-weight: 500; color: var(--brand-dark); margin-top: 3rem; line-height: 1.6; }
                    .animated-divider { height: 2px; width: 100px; background-color: var(--brand-primary); margin: 3rem auto; transform: scaleX(0); transform-origin: center; transition: transform 0.8s cubic-bezier(0.16, 1, 0.3, 1); }
                    .visible .animated-divider { transform: scaleX(1); }
            ...
            <header className="header">
                <nav className="nav-container">
                    ...
                    <div className="nav-links-center">
                        ...
                        {/* REWORKED: Dropdown now uses a delayed hover for a professional, reliable experience */}
                        <div 
            ...
            <main className="main-content">
                <section className="works-section">
                    <h1 className="animate-entry">
                        {"Driving Brand Growth & Business Success".split(" ").map((word, index) => (
                                <span key={index} className="word-wrapper">
                                    <span className={`animate-word ${word === 'Growth' || word === 'Success' ? 'highlight' : ''}`} style={{ transitionDelay: `${index * 80}ms`}}>
                                        {word}&nbsp;
                                    </span>
                                </span>
                        ))}
                    ...
                    <p className="animate-entry" style={{ transitionDelay: '300ms' }}>
                        At AffixxMedia, we have partnered with <strong>50+ brands</strong> across diverse industries—including clothing, education, restaurants, cafes, and resorts—delivering measurable results and an average of <strong>3X revenue growth</strong> for our clients. Our focus is on creating lasting brand impact and connecting businesses directly with their consumers through innovative marketing strategies.
                    </p>

                    <div className="animated-divider animate-entry" style={{ transitionDelay: '400ms' }}></div>

                    <p className="animate-entry" style={{ transitionDelay: '500ms' }}>
                        Every project begins with a deep dive into the brand’s vision, target audience, and market landscape. Using tailored solutions in social media marketing, brand strategy, and consumer engagement, we empower businesses to strengthen their online presence, expand their reach, and maximize sustainable growth.
                    </p>
                    
                    <p className="animate-entry" style={{ transitionDelay: '600ms' }}>
                        Our dedicated team delivers comprehensive resources for each campaign, from strategic planning to flawless execution. We ensure every effort drives brand awareness, consumer loyalty, and tangible revenue results. We don't just grow numbers; we foster meaningful connections between brands and their communities.
                    </p>

                    <p className="subtitle animate-entry" style={{ transitionDelay: '700ms' }}>
                        At AffixxMedia, we are committed to turning bold ideas into impactful realities. By combining creativity, strategy, and data-driven execution, we help businesses skyrocket their growth and achieve long-term success in today’s competitive market.
                    </p>
                </section>
            </main>
