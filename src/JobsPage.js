...
const JobsPage = () => {
    // This scrolls the window to the top every time the component is rendered.
    useEffect(() => {
        window.scrollTo(0, 0);
    }, []); // Empty dependency array ensures it runs only on the initial mount.
    ...
    const handleSubmit = (e) => {
        ...
        emailjs.send(serviceID, templateID, templateParams, publicKey)
            .then((response) => {
                ...
                setFormStatus({ message: 'Thank you! Your application has been submitted successfully.', type: 'success' });
                setFormData({ name: '', email: '', phone: '', portfolio: '', role: '', about: '' });
            })
            .catch((err) => {
                console.error('FAILED...', err);
                setFormStatus({ message: 'An error occurred. Please check your details and try again.', type: 'error' });
            })
            .finally(() => {
    ...
    const servicesLinks = [
        ...
        { name: 'Influencer Marketing', path: '/InfluenceMarketing', description: 'Leverage creators for authentic reach.', icon: <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M17.982 18.725A7.488 7.488 0 0012 15.75a7.488 7.488 0 00-5.982 2.975m11.963 0a9 9 0 10-11.963 0m11.963 0A8.966 8.966 0 0112 21a8.966 8.966 0 01-5.982-2.275M15 9.75a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 13.5L18 12.75l-1.5 1.5-1.5-1.5-1.5 1.5-1.5-1.5-1.5 1.5-1.5-1.5-1.5 1.5-1.5-1.5-1.5 1.5" stroke="none" /><path strokeLinecap="round" strokeLinejoin="round" d="M16.5 13.5L15 12l-1.5 1.5" stroke="none" /><path strokeLinecap="round" strokeLinejoin="round" d="M14.25 12.75l-1.5-1.5-1.5 1.5" stroke="none" /><path strokeLinecap="round" strokeLinejoin="round" d="M12.75 12l-1.5-1.5-1.5 1.5" stroke="none" /><path strokeLinecap="round" strokeLinejoin="round" d="M11.25 12.75l-1.5-1.5-1.5 1.5" stroke="none" /><path strokeLinecap="round" strokeLinejoin="round" d="M9.75 12l-1.5-1.5-1.5 1.5" stroke="none" /><path strokeLinecap="round" strokeLinejoin="round" d="M8.25 12.75L6.75 12l-1.5 1.5" stroke="none" /><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 15.75l-1.5-1.5-1.5 1.5" stroke="none" /><path strokeLinecap="round" strokeLinejoin="round" d="M13.5 15l-1.5-1.5-1.5 1.5" stroke="none" /><path strokeLinecap="round" strokeLinejoin="round" d="M12 15.75l-1.5-1.5-1.5 1.5" stroke="none" /><path strokeLinecap="round" strokeLinejoin="round" d="M10.5 15l-1.5-1.5-1.5 1.5" stroke="none" /><path strokeLinecap="round" strokeLinejoin="round" d="M9 15.75l-1.5-1.5-1.5 1.5" stroke="none" /><path strokeLinecap="round" strokeLinejoin="round" d="M15 18l-1.5-1.5-1.5 1.5" stroke="none" /><path strokeLinecap="round" strokeLinejoin="round" d="M12.75 17.25l-1.5-1.5-1.5 1.5" stroke="none" /><path strokeLinecap="round" strokeLinejoin="round" d="M11.25 18l-1.5-1.5-1.5 1.5" stroke="none" /><path strokeLinecap="round" strokeLinejoin="round" d="M9.75 17.25l-1.5-1.5-1.5 1.5" stroke="none" /><path strokeLinecap="round" strokeLinejoin="round" d="M12 21l-1.5-1.5-1.5 1.5" stroke="none" /></svg> }
    ];
        ...
        <div className="app">
            <style>
                {`
                    ...
                    /* --- GLOBAL & COMMON STYLES (Unchanged) --- */
                    ...
                    /* --- JOBS PAGE STYLES (Unchanged) --- */
                    .jobs-section { padding: 3rem 1.5rem; max-width: 900px; margin: 0 auto; line-height: 1.6; width: 100%; box-sizing: border-box; }
                    .jobs-section h1 { font-size: clamp(2.5rem, 6vw, 4.0625rem); font-weight: 600; color: var(--brand-primary); margin: 0 0 1rem; }
                    .jobs-section .header-text { font-size: 20px; color: var(--brand-dark); font-weight: 400; margin-bottom: 3rem; max-width: 800px; }
                    .jobs-section .heading-thin-black { color: var(--brand-dark); font-weight: 300; }
                    .application-form { display: flex; flex-direction: column; gap: 2rem; padding-bottom: 3rem; }
                    .form-group { display: flex; flex-direction: column; }
                    .form-group label { font-size: clamp(18px, 4.5vw, 20px); font-weight: 600; color: var(--brand-dark); margin-bottom: 0.5rem; text-transform: uppercase; letter-spacing: 0.05em; }
                    .form-group .sub-label { font-size: 14px; font-weight: 400; color: #6B7280; margin-top: 0.25rem; text-transform: none; letter-spacing: normal; }
                    .form-group input, .form-group textarea { padding: 0.75rem 0; font-size: clamp(16px, 4vw, 18px); border: none; border-bottom: 2px solid var(--divider-gray); background: transparent; outline: none; color: var(--brand-dark); font-family: 'Gilroy', sans-serif; transition: border-bottom-color 0.3s ease; font-weight: 500; }
                    .form-group input:focus, .form-group textarea:focus { border-bottom-color: var(--brand-primary); }
                    .form-group textarea { resize: vertical; min-height: 80px; }
                    .submit-button { align-self: flex-start; font-weight: 700; font-size: 18px; padding: 1rem 2rem; background-color: var(--brand-primary); color: var(--brand-light); border-radius: 4px; box-shadow: 0 4px 10px rgba(37, 164, 173, 0.4); border: none; cursor: pointer; transition: background-color 0.3s ease, box-shadow 0.3s ease; text-transform: uppercase; letter-spacing: 0.1em; width: 150px; text-align: center; }
                    /* REMOVED: Hover state handled by Framer Motion */
                    .form-status { padding: 1rem; margin-top: 1.5rem; border-radius: 4px; font-weight: 500; text-align: center; }
                    .form-status-success { background-color: #d1fae5; color: #065f46; }
                    .form-status-error { background-color: #fee2e2; color: #991b1b; }
                    .submit-button:disabled { background-color: #9ca3af; box-shadow: none; cursor: not-allowed; }
            ...
            <header className="header">
                {/* Header JSX with motion enhancements */}
                        ...
                        <div className="services-dropdown-container" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
                            ...
                            <AnimatePresence>
                                {isServicesDropdownOpen && (
                                    <motion.div className="dropdown-menu" initial={{ opacity: 0, y: -10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -10, scale: 0.95, transition: { duration: 0.2 } }}>
                    ...
                    <div className="nav-right">
                        ...
                        <div className="mobile-menu-button-container">
                            <button onClick={toggleMobileMenu} className="mobile-menu-button">
                                <svg className="hamburger-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" /></svg>
                            </button>
                        </div>
                    </div>
                </nav>
                    ...
                    {isMobileMenuOpen && (
                        <motion.div className="mobile-menu-backdrop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={toggleMobileMenu}>
                            <motion.div className="mobile-menu-panel" initial={{ x: '100%' }} animate={{ x: '0%' }} exit={{ x: '100%' }} transition={{ type: 'spring', stiffness: 300, damping: 30 }} onClick={(e) => e.stopPropagation()}>
                                ...
                                <div className="mobile-menu-header"><Link to="/AffixMedia" className="logo" onClick={toggleMobileMenu}>AffixxMedia</Link><button onClick={toggleMobileMenu} className="close-menu-button" aria-label="Close menu"><svg xmlns="http://www.w3.org/2000/svg" className="close-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></button></div>
                                <ul className="mobile-nav-links"><li><Link to="/AffixMedia" onClick={toggleMobileMenu}>Home</Link></li><li><button className="mobile-services-toggle" onClick={() => setIsMobileServicesOpen(!isMobileServicesOpen)}><span>Services</span><svg xmlns="http://www.w3.org/2000/svg" className={`chevron-icon ${isMobileServicesOpen ? 'open' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7" /></svg></button><ul className={`mobile-submenu ${isMobileServicesOpen ? 'open' : ''}`}>{servicesLinks.map(service => (<li key={service.name}><Link to={service.path} onClick={toggleMobileMenu}>{service.name}</Link></li>))}</ul></li><li><Link to="/WorksPage" onClick={toggleMobileMenu}>Works</Link></li><li><a href="/BlogPage" onClick={toggleMobileMenu}>Blogs</a></li><li><Link to="/JobsPage" onClick={toggleMobileMenu}>Jobs</Link></li></ul>
            ...
            <main className="main-content">
                ...
                >
                    ...
                    <motion.p className="header-text" variants={fadeInUp}>
                    ...
                    <motion.form 
                        ...
                        variants={staggerContainer} // Stagger children of the form
                    >
                        ...
                        <motion.div className="form-group" variants={fadeInUp}>
                            ...
                            <input type="url" id="portfolio" name="portfolio" value={formData.portfolio} onChange={handleFormChange} required />
                            <span className="sub-label">(No Google Drive links)*</span>
                        </motion.div>

                        <motion.div className="form-group" variants={fadeInUp}>
                            <label htmlFor="role">ROLE YOU ARE APPLYING FOR*</label>
                            <input type="text" id="role" name="role" value={formData.role} onChange={handleFormChange} required />
                        </motion.div>

                        <motion.div className="form-group" variants={fadeInUp}>
                            <label htmlFor="about">TELL US ABOUT YOURSELF</label>
                            <textarea id="about" name="about" value={formData.about} onChange={handleFormChange} required />
                        ...
                        <motion.div variants={fadeInUp}>
                            <motion.button 
                                ...
                                whileHover={{ scale: 1.05, boxShadow: '0 6px 15px rgba(37, 164, 173, 0.6)' }}
                                whileTap={{ scale: 0.95 }}
                            >
                        ...
                        <AnimatePresence>
                            {formStatus.message && (
                                <motion.div
                                    ...
                                    exit={{ opacity: 0, y: 10 }}
                                    className={`form-status ${formStatus.type === 'success' ? 'form-status-success' : 'form-status-error'}`}
                                >
                                    {formStatus.message}
                                </motion.div>
                            )}
